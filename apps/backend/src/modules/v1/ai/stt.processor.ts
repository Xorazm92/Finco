...
export class SttProcessor {
  ...
  async handleTranscriptionJob(
    ...
    this.logger.log(
      `Processing STT job ${job.id} for message ${job.data.messageLogId}`,
    );
    ...
    try {
      ...
      this.logger.log(
        `Transcription successful for message ${job.data.messageLogId}: ${text.substring(0, 50)}...`,
      );
    } catch (error) {
      this.logger.error(
        `Failed to transcribe audio for job ${job.id}: ${error.message}`,
        error.stack,
      );
      throw error;
    }
