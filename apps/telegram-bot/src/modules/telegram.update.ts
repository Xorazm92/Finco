...
export class TelegramUpdate {
  ...
  @On('message')
  async onMessage(@Message() message: unknown, @Ctx() ctx: Context) {
    // Type guard for message
    if (typeof message !== 'object' || message === null) return;
    const msg = message as { reply_to_message?: unknown; text?: string };
    await this.responseTimeService.processMessage(msg, ctx);

    // Avtomatik SI tahlil: reply bo'lgan va matni bor xabarlar
    if (msg.reply_to_message && typeof msg.text === 'string' && msg.text) {
      try {
        await this.aiQueueService.addSentimentJob(msg.text);
        // (Optional) Job IDni loglash yoki admin kanaliga yuborish mumkin
      } catch (e) {
        ctx.reply?.('AI navbatiga yuborishda xatolik yuz berdi.');
      }
    }
  }
  ...
  async analyzeSentiment(@Ctx() ctx: Context) {
    ...
    try {
      ...
      ctx.reply(
        `Sizning so'rovingiz (ID: ${job?.id ?? '?'} ) navbatga qo'yildi. Natijani /get_analysis_result ${job?.id ?? '?'} orqali tekshirishingiz mumkin.`,
      );
    } catch (e) {
      ctx.reply?.('AI navbatiga yuborishda xatolik yuz berdi.');
    }
  ...
  async assignRole(@Ctx() ctx: Context, @Message('text') text: string) {
    ...
    // Eski rolni aniqlash
    const oldRole = await /* this.userService. */ getUserRoleInChat(
      user.id,
      chatId,
    );
  ...
  async auditLog(@Ctx() ctx: Context) {
    ...
    if (args.length < 2)
      return ctx.reply(
        'Foydalanuvchini kiriting: /audit_log <user_id|username>',
      );
    const target = args[1];
    // Try by user id or username
    const logs = await this.auditLogService.getLogsByUser(target, 10);
  ...
  async auditLogLast(@Ctx() ctx: Context) {
    ...
    const userId = ctx.from?.id
      ? await /* this.userService. */ getUserIdByTelegram(ctx.from.id)
      : null;
    ...
    if (
      !user ||
      !userRole ||
      !['ADMIN', 'SUPERVISOR'].includes(String(userRole))
    )
  ...
  async checkIn(@Ctx() ctx: Context) {
    ...
    if (!log)
      return ctx.reply(
        'Siz bugun allaqachon check-in qilgansiz yoki foydalanuvchi topilmadi.',
      );
    return ctx.reply(
      log.isLate
        ? 'Check-in qabul qilindi, ammo siz kechikdingiz.'
  ...
  async checkOut(@Ctx() ctx: Context) {
    ...
    if (!log)
      return ctx.reply(
        'Siz hali check-in qilmagansiz yoki foydalanuvchi topilmadi.',
      );
  ...
  async submitReport(@Ctx() ctx: Context) {
    ...
    if (!args || args.length < 2)
      return ctx.reply(
        'Foydalanish: /submit_report <hisobot_kodi> [fayl biriktiring]',
      );
    ...
    if (!telegramMessage || !(telegramMessage as any).document)
    ...
    try {
      ...
      ctx.reply(
        'Hisobotni qabul qilishda xatolik: ' +
  ...
  async myKpi(@Ctx() ctx: Context) {
    ...
    const userId = ctx.from?.id
      ...
      : null;
    if (!userId) return ctx.reply('Foydalanuvchi topilmadi.');
    await this.kpiCalculationService.calculateResponseTimeKpisForUser(
      userId,
      weekAgo,
      now,
    );
    await this.kpiCalculationService.calculateReportSubmissionKpisForUser(
      userId,
      weekAgo,
      now,
    );
    await this.kpiCalculationService.calculateAttendanceKpisForUser(
      userId,
      weekAgo,
      now,
    );
    const kpiScores =
      await this.kpiCalculationService.getLatestKpiScoresForUser(userId, 10);
    const avgResp = kpiScores.find(
      (k) => k.kpiMetricCode === 'AVG_RESPONSE_TIME',
    );
    const percent10 = kpiScores.find(
      (k) => k.kpiMetricCode === 'PERCENT_RESPONSE_UNDER_10MIN',
    );
    const reportPercent = kpiScores.find(
      (k) => k.kpiMetricCode === 'REPORTS_ON_TIME_PERCENT',
    );
    const attendancePercent = kpiScores.find(
      (k) => k.kpiMetricCode === 'ATTENDANCE_ON_TIME_PERCENT',
    );
    const msg =
      `🕒 Sizning KPI:
` +
  ...
  async kpiStats(@Ctx() ctx: Context) {
    ...
    await this.kpiCalculationService.calculateAllUsersResponseTimeKpis(
      weekAgo,
      now,
    );
    ...
    if (userId) {
      ...
      const kpiScores =
        await this.kpiCalculationService.getLatestKpiScoresForUser(userId, 2);
      if (Array.isArray(kpiScores) && kpiScores.length) {
        const avgResp = kpiScores.find(
          (k) => k.kpiMetricCode === 'AVG_RESPONSE_TIME',
        );
        const percent10 = kpiScores.find(
